...
simulate <- function(
  ...
  sim.matrix <- data.frame(matrix(NA, nrow = time, ncol = 10))
...
plot2 <- function(sim_data, icu_beds) {
  ## Visualizing prevalence:
...
plot_backtest <- function(sim_data, x_lim) {
  
  sim <- simulate(initial = initial)
...
economy <- function(pop, sim_data) {
  
  population <- pop 
  
  economy_base <- simulate(initial = initial, quar.rate.i = 1) %>% # use only the prevalence columns
  ...
  economy_sim <- sim_data %>% # use only the prevalence columns
    select(time, i.num, h.num, f.num, q.num, qi.num) %>% 
    # examine only the first 100 days since it is all over by
    # then using the default parameters
    filter(time <= 360)
  ...
  economy_plot_df %>% ggplot(aes(x = time, y = count, colour = Groups)) + 
    geom_line(size = 2, alpha = 0.7) + scale_colour_manual(values = compcols, 
                                                           labels = complabels) + theme_dark() + labs(title = "GDP Impacto: Baseline simulation vs Quarantine", 
...
server <- function(input, output) {
  ...
  output$plot3 <- renderPlot({
    ...
    economy(pop = input$input_pop, sim_data = simulate(
      initial = initial <- c(
        time = 1,
        s.num = input$input_pop,
        e.num = input$input_popinf,
  ...
  output$plot4 <- renderPlot({
    ...
    plot_backtest(sim_data = simulate(
      initial = initial <- c(
        time = 1,
        s.num = input$input_pop,
        e.num = input$input_popinf,
